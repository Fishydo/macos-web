...
<style>
	...
	.titlebar { display: flex; align-items: center; padding-left: 5rem; }
	h1 { font-size: 1rem; color: var(--system-color-light-contrast); }
	.main-area { padding: 1rem; display: grid; grid-template-columns: 1fr; gap: 1rem; overflow: auto; }
	.install-card { padding: 1rem; border-radius: 0.8rem; background: hsla(var(--system-color-dark-hsl), 0.08); display: grid; gap: 0.8rem; }
	.install-row { display: grid; grid-template-columns: 1fr auto; gap: 0.5rem; }
	...
	button { border-radius: 0.5rem; padding: 0.5rem 0.9rem; background: hsl(210deg 90% 52%); color: white; font-weight: 500; }
	.apps-list { display: grid; gap: 0.6rem; }
	h2 { font-size: 0.95rem; font-weight: 600; color: var(--system-color-light-contrast); }
	.empty,.error,.url { color: hsla(var(--system-color-dark-hsl), 0.7); font-size: 0.85rem; }
	.installed-app { display: grid; grid-template-columns: 2.3rem 1fr; gap: 0.7rem; text-align: left; background: hsla(var(--system-color-light-hsl), 0.55); color: var(--system-color-light-contrast); align-items: center; }
	.installed-app img { width: 2.3rem; height: 2.3rem; border-radius: 0.55rem; object-fit: cover; }
