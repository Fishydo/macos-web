...
async function getSharedConnection() {
    ...
    const candidates = [...new Set([current, ...getAllWispServers().map((server) => server.url)])].filter(Boolean);
    let lastError = null;

    for (const wispUrl of candidates) {
        try {
            const nextConnection = new BareMux.BareMuxConnection(basePath + "bareworker.js");
            await nextConnection.setTransport(
                "https://cdn.jsdelivr.net/npm/@mercuryworkshop/epoxy-transport@2.1.28/dist/index.mjs",
                [{ wisp: wispUrl }]
            );
            sharedConnection = nextConnection;
    ...
    throw lastError || new Error("No working wisp server available");
}
